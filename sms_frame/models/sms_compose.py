...
class SmsCompose(models.Model):
    ...
    def _onchange_sms_template_id(self):
        ...
        if self.sms_template_id.id != False:
            sms_rendered_content = self.env['sms.template'].render_template(self.sms_template_id.template_body,
    ...
    def send_entity(self):
        ...
        if self.delivery_time:
            ...
            sms_message = self.env['sms.message'].create({'record_id': self.record_id, 'model_id': my_model[0].id,'account_id': self.from_mobile_id.account_id.id,'from_mobile': self.from_mobile_id.mobile_number,'to_mobile': self.to_number, 'sms_content': self.sms_content,
            ...
            my_sms = self.from_mobile_id.account_id.send_message(self.from_mobile_id.mobile_number, self.to_number,
                                                                 self.sms_content.encode('utf-8'), self.model,
                                                                 self.record_id, self.media_id,
        ...
        if my_sms.delivary_state == "failed":
            return {
                ...
                'view_mode': 'form',
                'target': 'new',
                'context': {'default_to_number': self.to_number, 'default_record_id': self.record_id,
                            'default_model': self.model, 'default_error_message': error_message}
            }
        else:
            ...
            sms_message = self.env['sms.message'].create({'record_id': self.record_id, 'model_id': my_model[0].id,
                                                          ...
                                                          'direction': 'O', 'message_date': datetime.utcnow(),
            ...
            # ashraf this is commented for bodaring for model except partner
        # Probably it send email to followers about sms
