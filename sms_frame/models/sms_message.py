...
class SmsMessage(models.Model):
    ...
    status_code = fields.Selection((('RECEIVED', 'Received'), ('failed', 'Failed to Send'), ('queued', 'Queued'),
                                    ('successful', 'Sent'), ('DELIVRD', 'Delivered'), ('EXPIRED', 'Timed Out'),
                                    ('UNDELIV', 'Undelivered')), string='Delivary State', readonly=True)
    ...
    message_date = fields.Datetime(string="Send/Receive Date", readonly=True,
                                   help="The date and time the sms is received or sent")
    ...
    def process_sms_queue(self, queue_limit):
        # queue_limit = self.env['ir.model.data'].get_object('eagle_sms', 'sms_queue_check').args
        for queued_sms in self.env['sms.message'].search([('status_code', '=', 'queued'), (
        'message_date', '<=', datetime.today().strftime(DEFAULT_SERVER_DATETIME_FORMAT))], limit=queue_limit):
            gateway_model = queued_sms.account_id.account_gateway_id.gateway_model_name
            my_sms = queued_sms.account_id.send_message(queued_sms.from_mobile, queued_sms.to_mobile,
                                                        queued_sms.sms_content.encode('utf-8'),
                                                        queued_sms.model_id.model, queued_sms.record_id,
                                                        queued_sms.media_id,
