...
class SmsGatewaySSL(models.Model):

    _name = "sms.gateway.ssl"
    _description = "SSL SMS Gateway"
    ...
    def send_message(self, sms_gateway_id, from_number, to_number, sms_content, my_model_name='', my_record_id=0,
        ...
        # if media:
        #
         #    attachment_id = self.env['ir.attachment'].sudo().create({'name': 'mms ' + str(my_record_id),
         #        'type': 'binary', 'datas': media, 'public': True, 'mms': True, 'datas_fname': media_filename})
        #
        ...
        #send the sms/mms
        #payload = { "sms[0][0]" : format_to, "sms[0][1]": sms_content, 'sid':'DSBLSC','user':'dsblsc','pass':'SSl@1234'}
        payload = { "sms[0][0]" : format_to, "sms[0][1]": sms_content, sms_account.param_sid:sms_account.sid,sms_account.param_user:sms_account.user,sms_account.param_pass:sms_account.password}
        ...
        # print(payload)
        response_string = requests.post("http://sms.sslwireless.com/pushapi/dynamic/server.php" , data=payload, )
        ...
        print(root)
        my_elements_human = root.xpath('//LOGIN')
        ...
        my_elements = root.xpath('//REFERENCEID')
